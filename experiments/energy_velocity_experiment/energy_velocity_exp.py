...
sys.path.append(os.path.abspath("/media/ruben/data/documents/unief/thesis/thesis_manta_ray"))
...
for reward_fn in DragRaceTask.reward_functions:
    for velocity in list(np.linspace(0.2, 2., 1)):
        # morphology
        ...
        cma = CMA(mean=np.random.uniform(low=0,
                ...
                sigma=0.005,
                bounds=bounds,
                population_size=2,    # has to be more than 1
                lr_adapt=True,
                )
        sim = OptimizerSimulation(
            ...
            population_size=2,  # make sure this is a multiple of num_envs
            num_generations=1,
    ...
    sim.finish(store=True, name="energy_velocity_experiment/sim_objects/energy_velocity_"+str(velocity)+"_reward_function_"+reward_fn)