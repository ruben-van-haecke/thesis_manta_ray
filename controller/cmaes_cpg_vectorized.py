...
class CPG():
    ...
    def ask(self,
        ...
        def dynamics(t, y):
            ...
            print( self._a_r * (self._a_r/4*(self._r[0, :]-r)-r_dot))
        ...
        if time == 0:   # reset for new episode
            ...
            self._last_state[0*self._num_neurons: 1*self._num_neurons] = np.random.normal(loc=0, scale=0.1, size=self._num_neurons)
        ...
        if duration == None:    # only one time step
            ...
            self._last_time = time
            out = sol.y[1*self._num_neurons:2*self._num_neurons, :] * np.sin(sol.y[0:self._num_neurons, :]*time + sol.y[2*self._num_neurons:3*self._num_neurons, :])
        else:   # multiple time steps i.e. 2 until the end of the episode
            sol = solve_ivp(fun=dynamics,
            ...
            self._last_time = time
            t_eval=np.linspace(time, time+duration-sampling_period, int(duration/sampling_period))
            out = sol.y[1*self._num_neurons:2*self._num_neurons, :] * np.sin(sol.y[0:self._num_neurons, :]) + sol.y[2*self._num_neurons:3*self._num_neurons, :]
        return out
